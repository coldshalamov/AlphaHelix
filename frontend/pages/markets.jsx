...
const PAGE_SIZE = 10; // BOLT: Added fixed page size to prevent RPC payload explosion
...
export default function MarketsPage() {
  ...
  // BOLT: Reset page if out of bounds (e.g. network switch causes count to drop)
  useEffect(() => {
  ...
  const { data: marketsResults, isLoading, error: queryError } = useReadContracts({
    contracts: useMemo(() => {
      ...
      const count = Math.min(PAGE_SIZE, numericCount - start);
      if (count <= 0) return [];
    ...
    <div className="grid section">
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', marginTop: '2rem' }}>
          ...
          <span style={{ alignSelf: 'center', color: 'var(--color-text-secondary)' }}>Page {page + 1} of {totalPages}</span>
          <button
            className="button secondary"
