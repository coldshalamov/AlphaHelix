...
function BettingWidget({
  ...
  const handleAmountChange = useCallback((e) => {
    ...
    // Replace commas with dots and strip non-numeric characters for international keyboards
    val = val.replace(/,/g, '.').replace(/[^\d.]/g, '');
    // Prevent multiple decimal points
    const parts = val.split('.');
    if (parts.length > 2) val = parts[0] + '.' + parts.slice(1).join('');

    // Force DOM update to avoid desync when invalid characters are stripped
