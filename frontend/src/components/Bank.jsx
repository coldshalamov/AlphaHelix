...
import { VALID_DECIMAL_REGEX, INPUT_SANITIZATION_REGEX, MAX_INPUT_LENGTH } from '@/lib/constants';
...
function Bank() {
  ...
  const handleBuyAmountChange = useCallback((e) => {
    ...
    if (val === '' || VALID_DECIMAL_REGEX.test(val)) {
  ...
  const handleBuy = useCallback(async () => {
    ...
    if (!INPUT_SANITIZATION_REGEX.test(buyAmount)) {
  ...
  const handleSell = useCallback(async () => {
    ...
    if (!INPUT_SANITIZATION_REGEX.test(sellAmount)) {
      setStatus('Invalid amount format.');
      setActiveAction(null);
      return;
    }
