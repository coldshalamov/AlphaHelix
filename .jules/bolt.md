...
**Action:** Use native utilities like `bytesToHex` from `viem` for optimal memory/CPU performance on the frontend. Note that `bytesToHex` natively returns a `0x`-prefixed string, so do not manually prepend `'0x' +`.
