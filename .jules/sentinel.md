...
## 2026-09-08 - Prevent Randomness Grinding via Sender
**Vulnerability:** The `HelixMarket` contract calculates a global closeHash using `msg.sender` to determine if a market randomly closes. This allowed attackers to grind the hash using proxy contracts (or CREATE2) until a favorable hash triggered the random close.
**Learning:** Never include user-controllable variables like `msg.sender` or `tx.origin` in the entropy pool for global randomness logic. This prevents malicious manipulation of the random outcomes.
**Prevention:** Remove user-controllable properties from `keccak256` randomness calculations.
