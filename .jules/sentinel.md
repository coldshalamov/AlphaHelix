...
## 2024-09-06 - Remove msg.sender from randomness entropy
**Vulnerability:** Included `msg.sender` in global on-chain randomness calculation for market closures.
**Learning:** Including user-controllable variables in global entropy pools allows attackers to grind randomness via proxy contracts or `CREATE2` addresses to force desired outcomes.
**Prevention:** Never include user-controllable variables like `msg.sender` or `tx.origin` in entropy pools when calculating on-chain global randomness.
