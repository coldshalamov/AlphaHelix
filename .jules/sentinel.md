...
## 2026-01-15 - Prevent DoS via Atomicity on Optional Reward Transfers
**Vulnerability:** A reverting `require()` statement was used for transferring the optional `PING_REWARD` at the end of core logic functions (`commitBet`, `revealBet`, `pingMarket`).
**Learning:** If an optional or auxiliary external call (like a reward transfer) fails, it reverts the entire transaction. This could cause a Denial of Service, bricking core market functionality if the market contract had insufficient balance or the token transfer otherwise failed.
**Prevention:** Use a `try/catch` block for non-critical external calls at the end of core operations to safely handle failures without reverting the main transaction.
