...
## 2025-06-03 - [Revert Loop Vulnerability in Core Business Logic]
**Vulnerability:** The functions `commitBet`, `revealBet`, `resolve`, and `pingMarket` perform external calls to `token.transfer` at the end of their logic. If these external calls fail (e.g., due to a paused token contract or blacklisted address), the entire transaction reverts, causing a Denial-of-Service (DoS) and bricking the core business logic.
**Learning:** Avoid using reverting `require()` statements for non-critical auxiliary operations (like optional reward/fee transfers) at the end of core logic functions. If the external call fails, it reverts the entire transaction.
**Prevention:** Use a `try/catch` block or low-level calls to safely handle failures without blocking primary operations.
