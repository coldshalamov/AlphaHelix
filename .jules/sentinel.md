...
## 2024-04-13 - Weak Pseudo-Randomness Seed Generation
**Vulnerability:** The `s.closeSeed` in `HelixMarket` was generated using only predictable elements (`block.timestamp`, `msg.sender`, `marketId`, `blockhash(block.number - 1)`), making it vulnerable to manipulation by validators.
**Learning:** In PoS Ethereum, legacy block properties are easier to grind. A stronger source of entropy is required.
**Prevention:** Always include `block.prevrandao` in the `keccak256` payload for stronger pseudo-randomness generation.
