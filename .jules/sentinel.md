...
## 2025-07-04 - Denial of Service via Atomicity in Auxiliary Operations
**Vulnerability:** In `HelixMarket.sol`, external calls like `token.transfer` used for auxiliary operations (such as paying rewards or fees) were wrapped in `require()` statements. If the transfer failed, the entire transaction reverted, causing a Denial of Service (DoS) for core functionalities like `commitBet`, `revealBet`, and `resolve`.
**Learning:** Using `require()` for non-critical token transfers (e.g., ping rewards, originator fees) makes the core contract logic brittle and susceptible to DoS. An attacker or even a simple contract failure could prevent markets from resolving or bets from being revealed.
**Prevention:** Use `try/catch` blocks for auxiliary external calls where failure should not block primary operations, ensuring graceful degradation.
