...
## 2026-07-04 - Fix Denial of Service (DoS) via Reverting External Calls
**Vulnerability:** Core logic functions (`commitBet`, `revealBet`) and `pingMarket` were using reverting `require(token.transfer(...))` for optional PING_REWARD payouts at the end of their logic. If the external transfer call reverted (e.g. out of gas or token blocklist), the entire transaction would revert, effectively bricking the core functions for users in those markets.
**Learning:** Reverting external token transfers at the end of critical state-modifying functions introduces a Denial of Service risk because atomicity ensures the entire transaction is rolled back. Non-critical reward transfers should not block essential business logic.
**Prevention:** Always use safe low-level calls or `try-catch` blocks to swallow errors for non-essential external transfers (like optional rewards) ensuring that the core transaction state changes are preserved even if the auxiliary transfer fails.
