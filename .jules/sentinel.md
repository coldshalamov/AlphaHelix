...
## 2026-05-14 - State Machine Bypass via Uninitialized Timestamps
**Vulnerability:** In `HelixMarket.sol`, `withdrawUnrevealed` allowed users to bypass the commit phase lock for random close markets because `s.revealEndTime` is initially `0`, causing `block.timestamp > s.revealEndTime` to trivially pass.
**Learning:** When using timestamps for state transitions (e.g., `> s.revealEndTime`), if the target timestamp is initialized lazily (e.g., set to `0` until an event occurs), the check must explicitly ensure the timestamp is non-zero before comparison.
**Prevention:** Always validate that lazily initialized state variables (like timestamps) have been properly set (e.g., `require(time != 0)`) before using them in threshold comparisons.
