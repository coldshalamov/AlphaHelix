...
## 2024-05-26 - [Randomness Manipulation via msg.sender]
**Vulnerability:** The random close logic in `HelixMarket.sol` included `msg.sender` in the `keccak256` entropy pool. This allowed an attacker to grind the randomness by calling the contract from many different addresses (e.g., via proxy contracts or `CREATE2`) until a favorable `closeHash` was found, effectively manipulating market closure times.
**Learning:** Including any user-controllable variable (like `msg.sender` or `tx.origin`) in global on-chain randomness calculations compromises the randomness, as it allows attackers to search for inputs that produce desired outcomes without affecting the core protocol logic.
**Prevention:** Never include `msg.sender` or other user-controllable fields in the entropy pool for global/critical randomness. Rely on non-controllable sources like `block.prevrandao`.
