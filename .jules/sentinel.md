...
## 2026-04-21 - Secure Random Seed Generation
**Vulnerability:** Weak pseudo-randomness in `closeSeed` generation using only predictable variables (`blockhash`, `timestamp`, `msg.sender`).
**Learning:** In Ethereum PoS, generating random seeds using only predictable elements is vulnerable to manipulation by validators.
**Prevention:** Always include `block.prevrandao` in the `keccak256` payload for a stronger source of pseudo-randomness.
