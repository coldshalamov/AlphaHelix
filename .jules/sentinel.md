...
## $(date +%Y-%m-%d) - CI Node 16/20 Deprecation Remediation
**Vulnerability:** The CI pipeline suffered unexpected failures (403 Forbidden and ENOENT) and threw several critical runner warnings. These stemmed from the pipeline running heavily deprecated `v3`/`v6` GitHub Actions bound to outmoded versions of Node, and an execution environment that failed to gracefully handle file outputs during automated PR gas reporting.
**Learning:** Upgrading GitHub actions strictly to `v4`/`v7` natively resolves `Node 20 actions are deprecated` errors. Moreover, when using the newer `actions/github-script` to write PR comments, Explicit token permissions (`permissions: pull-requests: write, issues: write, contents: read`) must be injected at the job level. Finally, outputting data via standard `set -e` test execution can drop piped reports if not correctly configured, confirming `| tee gas-report.txt` correctly persists files without masking test failures.
**Prevention:** Maintain strict version pinning for standard actions (`checkout@v4`, `setup-node@v4`, `github-script@v7`) and ensure jobs writing back to the repository explicitly define minimum permissions required since the global token defaults to read-only.
