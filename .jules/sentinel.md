...
## 2025-05-23 - [Randomness Manipulation via msg.sender]
**Vulnerability:** `HelixMarket.sol` used `msg.sender` as an entropy source in `_checkRandomClose`. This allows a malicious actor to grind the randomness within a single block by deploying multiple proxy contracts to find an address that satisfies the difficulty target, forcing the market to close prematurely.
**Learning:** Including user-controllable variables (like `msg.sender`) in on-chain randomness calculations intended to be global compromises the integrity of the randomness, transforming a global random event into a user-grindable condition.
**Prevention:** Ensure that global, objective randomness sources only rely on non-user-controllable inputs such as `block.prevrandao`, `blockhash`, and immutable seeds.
