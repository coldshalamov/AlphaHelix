...
## 2026-05-24 - [CEI Violation in Event Emission]
**Vulnerability:** Multiple functions in `HelixMarket.sol` and `HelixReserve.sol` emitted events after making external token transfer calls. Although functions use `nonReentrant`, emitting events after external calls violates the Checks-Effects-Interactions (CEI) pattern. It can cause out-of-order event logs in complex transaction chains, complicating off-chain indexing and monitoring, and exposing edge-case logical reentrancy if the event data itself is relied upon.
**Learning:** Even with reentrancy guards, strictly adhering to the CEI pattern means that all local state changes, *including event emissions (Effects)*, must occur before any external calls (Interactions).
**Prevention:** Always emit events *before* interacting with external contracts (like `token.transfer` or `token.transferFrom`) to preserve deterministic event ordering and robust CEI structure.
