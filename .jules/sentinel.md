...
## 2024-06-28 - Avoid DoS via Reverting Require on Optional Token Transfers
**Vulnerability:** In `HelixMarket.sol`, optional reward transfers (`PING_REWARD`) at the end of core logic functions (`commitBet`, `revealBet`, `pingMarket`) were using reverting `require` statements.
**Learning:** If the contract lacks sufficient token balance for the reward, the `token.transfer` fails, causing the entire transaction to revert. This results in a Denial of Service (DoS) for core functions. Reverting `require()` statements for non-critical auxiliary operations at the end of core logic functions shouldn't be used.
**Prevention:** Use a `try/catch` block for external calls involving optional operations (like rewards) to safely handle failures without blocking primary operations. Always explicitly declare the return value in the `try` statement for external calls that return a value.
