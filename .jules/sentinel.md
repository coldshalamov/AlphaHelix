...
## 2024-06-14 - Fix Denial of Service via Atomicity in optional reward transfers
**Vulnerability:** Core logic functions (`commitBet`, `revealBet`, `resolve`, `pingMarket`) could revert entirely if the ERC20 token transfer for optional rewards (like `PING_REWARD` or originator `fee`) failed.
**Learning:** Reverting on non-critical side effects breaks core market functionality, leading to a Denial of Service.
**Prevention:** Always use `try/catch` blocks or low-level calls for optional, auxiliary external operations that should not block the primary business logic.
