...
## 2026-05-06 - Remove msg.sender from random hash to prevent grinding
**Vulnerability:** The random close mechanism in HelixMarket used `msg.sender` in its hash entropy. This allowed attackers to grind the hash by deploying smart contracts (via CREATE2) or using multiple addresses to find a hash that meets the difficulty target, forcing the market to close prematurely exactly when they want to.
**Learning:** Including user-controlled variables like `msg.sender` in randomness calculations enables grinding attacks within a single block, undermining the unpredictability of the mechanism.
**Prevention:** Only use block-level entropy (`blockhash`, `block.prevrandao`, `block.timestamp`) and fixed market state in randomness mechanisms where predictability must be bounded, ensuring the hash remains uniform across the entire block and cannot be manipulated per-call.
