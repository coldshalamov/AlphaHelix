...
## 2024-05-26 - [Logic DoS via Pre-execution Side Effects]
**Vulnerability:** The `commitBet` function executed a state-changing side-effect (`_checkRandomClose`) *before* validating the function's own preconditions (`require(commitPhaseOpen)`). If the side-effect closed the phase, the precondition check would immediately fail, causing the entire transaction to revert. This created a Denial-of-Service where users could not successfully execute a transaction that also triggered the random close.
**Learning:** Checking state preconditions *after* invoking state-altering internal functions or side-effects is extremely dangerous and can inadvertently block core functionality. The transaction fails because the side-effect changes the very state the function relies on to proceed.
**Prevention:** Always validate all function preconditions and require statements *before* calling internal functions that modify state, adhering strictly to the order of Operations (Preconditions -> State Changes -> External Calls).
