...
## 2026-05-27 - Enforce CEI pattern for event emissions
**Vulnerability:** Events were being emitted after external state-changing calls, which violates the Checks-Effects-Interactions (CEI) pattern.
**Learning:** Emitting events after external calls can lead to out-of-order logs in complex transaction chains and creates potential security risks even if `nonReentrant` is present.
**Prevention:** Always ensure `emit` statements occur before any external calls.
