...
## 2026-07-03 - DoS via Atomicity in Auxiliary Rewards
**Vulnerability:** A reverting `require()` statement was used for an auxiliary, optional token transfer (ping reward) at the end of core functions (`commitBet`, `revealBet`, `pingMarket`). If this external call failed for any reason, the entire transaction reverted, rendering the core logic unreachable and permanently DoS-ing the market.
**Learning:** Reverting on non-critical external calls violates resilience and can brick core logic. The failure of an optional reward should never block the primary state transition (like committing a bet or closing a market phase).
**Prevention:** Use a `try/catch` block or low-level calls to safely execute optional or auxiliary external calls without propagating the revert to the main transaction context.
