...
## 2024-06-15 - DoS via Atomicity in Auxiliary Transfers
**Vulnerability:** Reverting `require(token.transfer(...))` statements for non-critical auxiliary operations (like optional reward transfers and originator fees) at the end of core logic functions (e.g., market resolution and random closing).
**Learning:** If the external token transfer fails (e.g., due to insufficient contract balance or a blacklisted recipient), it reverts the entire transaction. This bricks core business logic, preventing markets from closing or resolving, which permanently locks user funds.
**Prevention:** Use a `try/catch` block for non-critical auxiliary token transfers to safely handle failures without blocking primary operations. Always explicitly declare the return value in the `try` statement (e.g., `returns (bool)`) to match the ERC20 interface.
