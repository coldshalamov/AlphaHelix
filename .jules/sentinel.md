...
## 2024-09-03 - [Randomness Manipulation via msg.sender]
**Vulnerability:** `HelixMarket.sol` included `msg.sender` in the `keccak256` entropy pool used to determine random market closures. This allowed attackers to grind the randomness by deploying proxy contracts via `CREATE2` to find a `msg.sender` that forces a favorable market close.
**Learning:** Any user-controllable variable (like `msg.sender` or `tx.origin`) introduced into an on-chain randomness function allows an attacker to manipulate the outcome by iterating over possible values off-chain before submitting the transaction.
**Prevention:** Never include `msg.sender`, `tx.origin`, or other user-controllable inputs when generating global, critical randomness. Rely solely on environmental block data like `block.prevrandao`.
