...
## 2025-02-23 - [DoS via Modifier State Transition Revert]
**Vulnerability:** In `HelixMarket.sol`, the `checkRandomClose` modifier sets `s.commitPhaseClosed` to `block.timestamp` when a random close is triggered. However, the modified function `commitBet` immediately required `s.commitPhaseClosed == 0`. This caused the `commitBet` transaction to revert if it triggered the random close, essentially rolling back the close and causing a Denial of Service (DOS) on the random close mechanism via bets.
**Learning:** When a modifier induces a state transition that the modified function also explicitly checks, the function must be designed to accommodate the new state. A failure to synchronize the expectations between the modifier and the function body can result in unintentional reverts and broken protocol mechanics.
**Prevention:** Ensure that functions modified by state-changing modifiers explicitly allow execution in the newly transitioned state (e.g., `s.commitPhaseClosed == block.timestamp` in the current block) if the function's logic intends to allow it.
