...
**Vulnerability:** The on-chain global randomness calculation for random market closures (`_checkRandomClose`) included `msg.sender` in its entropy pool (`keccak256`).
**Learning:** Including user-controllable variables like `msg.sender` or `tx.origin` in the global entropy pool allows attackers to grind the randomness by using proxy contracts or `CREATE2` to find a `msg.sender` address that forces the desired outcome.
**Prevention:** Never include user-controllable variables in global entropy pools. Use uncontrollable values like `block.prevrandao`.
