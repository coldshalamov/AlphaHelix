...
## 2026-04-25 - Use block.prevrandao for Randomness
**Vulnerability:** Weak random number generation using only predictable elements (e.g., blockhash, block.timestamp) can be manipulated.
**Learning:** In post-Merge Ethereum, `block.prevrandao` provides a stronger, more secure source of pseudo-randomness from the beacon chain.
**Prevention:** Always include `block.prevrandao` in `keccak256` payloads when generating random seeds for critical logic.
