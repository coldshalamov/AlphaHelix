...
## 2024-05-26 - [Transaction Revert on State Transition]
**Vulnerability:** In `HelixMarket.sol`, `commitBet` required the market to be open (`commitPhaseClosed == 0`). However, the `checkRandomClose` modifier could transition the market to closed (`commitPhaseClosed = now`). Since the modifier runs before the function body, the check inside `commitBet` would fail, causing the entire transaction to revert. This prevented the market from ever closing via `commitBet` and created a denial-of-service for users trying to bet near the close.
**Learning:** When a transaction triggers a state change (like closing a market) that invalidates its own preconditions, the transaction will revert, nullifying the state change. This can lead to "zombie states" where the transition condition is met but cannot be persisted by the primary interaction method.
**Prevention:** Ensure function preconditions account for state transitions that occur within the same transaction. For example, allow `commitBet` if the market is open OR if it was closed in the current block (`commitPhaseClosed == block.timestamp`).
