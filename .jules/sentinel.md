...
## 2024-09-19 - [CEI Violation in _submitStatementInternal]
**Vulnerability:** The `_submitStatementInternal` function in `HelixMarket.sol` performed an external call (`token.transferFrom`) before completing the initialization of state variables for the newly created market. This violates the Checks-Effects-Interactions pattern.
**Learning:** Even when `ReentrancyGuard` is used, performing external calls before state updates are complete is a security risk. If the external token has a malicious callback or hook, it could re-enter the contract before the market state is fully set.
**Prevention:** Always place external calls (Interactions) at the very end of the function, strictly after all Checks and state Effects have been completed.
