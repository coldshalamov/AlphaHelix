...
## 2024-05-26 - [Randomness Manipulation via msg.sender Grinding]
**Vulnerability:** `HelixMarket.sol` used `msg.sender` as an entropy source for `_checkRandomClose` and `previewCloseCheck`. This allowed attackers to grind `msg.sender` (by repeatedly simulating transactions or deploying smart contracts via `CREATE2` to find a specific sender address) to force a market close at a favorable time, effectively bypassing the unpredictability of the random close feature.
**Learning:** Any user-controllable input, including `msg.sender`, can be ground by a sophisticated attacker using smart contracts to find favorable hashes in a single block. While block properties (like `blockhash`, `prevrandao`) are less manipulable by standard users, the inclusion of `msg.sender` grants the attacker control over the entropy pool.
**Prevention:** Never include `msg.sender` or `tx.origin` in entropy pools intended for global, on-chain randomness if the outcome is binary/deterministic. Rely solely on external inputs (e.g., Oracles) or sufficiently difficult-to-manipulate block data (e.g., `prevrandao`).
