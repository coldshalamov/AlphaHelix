...
## 2026-05-19 - Fix CEI Violation in HelixReserve
**Vulnerability:** The `sell` function in `HelixReserve.sol` performed an external low-level call (`msg.sender.call`) before emitting the `Sold` event. This violated the Checks-Effects-Interactions (CEI) pattern.
**Learning:** While the function was protected by a `nonReentrant` modifier, emitting events after external interactions can lead to out-of-order event logs if reentrancy were to occur or in complex transaction chains, complicating off-chain indexing and monitoring.
**Prevention:** Always emit events (Effects) before making any external calls (Interactions) to strictly adhere to the CEI pattern and ensure state changes and event logs accurately reflect the contract's state before control is handed over to an external entity.
