...
## 2026-04-18 - [Weak Random Seed Generation]
**Vulnerability:** The initial `closeSeed` for random-close markets was generated using only predictable elements like `block.timestamp`, `msg.sender`, `marketId`, and `blockhash(block.number - 1)`. This weak randomness is vulnerable to manipulation by validators or advanced attackers.
**Learning:** In Ethereum Proof-of-Stake (post-Merge), generating random seeds without `block.prevrandao` exposes the protocol to predictability and manipulation. `blockhash` is no longer sufficient on its own.
**Prevention:** Always include `block.prevrandao` in `keccak256` payloads when a strong source of on-chain pseudo-randomness is required.
