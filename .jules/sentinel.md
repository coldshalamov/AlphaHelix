...
## 2024-05-18 - Prevent Randomness Grinding via User-Controllable Entropy
**Vulnerability:** The random close mechanism in HelixMarket used `msg.sender` as an entropy source to calculate the `closeHash`.
**Learning:** Including user-controllable values like `msg.sender` in global randomness calculations allows an attacker to grind for favorable hashes using proxy contracts or `CREATE2`, increasing their chances of forcefully closing a market at a specific time.
**Prevention:** Never include user-controllable variables like `msg.sender` or `tx.origin` in global entropy pools. Stick to less controllable sources like blockhashes or block.prevrandao.
