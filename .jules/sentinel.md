...
## 2026-05-03 - State Modification Order DoS in Commit Phase
**Vulnerability:** A Denial of Service (DoS) occurred in `commitBet` where calling a state-modifying check (`_checkRandomClose`) before verifying the phase was open caused transactions to revert exactly when they were supposed to trigger a phase transition.
**Learning:** State-modifying checks that alter the very conditions validated by subsequent `require` statements can inadvertently brick core functionality.
**Prevention:** Always perform prerequisite state validations (`require(isOpen)`) before executing subroutines that might legally transition that state (`checkAndClose()`), or cache the pre-state if the subroutine's execution is required first.
