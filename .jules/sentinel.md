...
## 2024-06-11 - [Denial of Service via Atomicity]
**Vulnerability:** Reverting `require()` statements were used for non-critical auxiliary operations (like `PING_REWARD` transfers and resolution fees) at the end of core logic functions. If the external token transfer failed, the entire transaction reverted, bricking core functionalities.
**Learning:** The atomicity of Ethereum transactions means that any revert bubbles up and cancels all preceding state changes. Auxiliary side-effects (like optional rewards or fees) must not be allowed to revert and block core business logic (like resolving a market).
**Prevention:** Avoid using reverting `require()` statements for non-critical auxiliary operations. Use a `try/catch` block or low-level calls to safely handle failures without blocking primary operations.
