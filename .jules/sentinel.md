...
## 2024-07-08 - [Denial of Service via Atomicity in Core Market Functions]
**Vulnerability:** The `require(token.transfer(msg.sender, PING_REWARD))` statement was used for non-critical auxiliary operations (optional reward transfers) at the end of core logic functions (`commitBet`, `revealBet`).
**Learning:** If the external token transfer fails (e.g., contract lacks balance, or receiver is blacklisted on a compliant token), it reverts the entire transaction, causing a Denial of Service (DoS) and completely bricking the core business logic of the prediction market.
**Prevention:** Always use a `try/catch` block for non-critical external calls, such as `try token.transfer(msg.sender, PING_REWARD) returns (bool) {} catch {}`, so that failures in auxiliary operations do not block the primary functionality.
