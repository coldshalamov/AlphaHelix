...
## 2024-05-26 - [Reverted State Change in Modifier]
**Vulnerability:** The `checkRandomClose` modifier in `HelixMarket.sol` updated the `commitPhaseClosed` state when a random condition was met. However, the modified function (`commitBet`) subsequently checked `require(commitPhaseClosed == 0)`. If the modifier closed the market, this check failed, causing the entire transaction (including the state change) to revert. This effectively prevented the market from ever closing via `commitBet`, creating a Denial of Service on the closing mechanism.
**Learning:** Modifiers that change state must be compatible with the function body they wrap. If a function assumes a certain state (like "market is open"), but a modifier changes that state (to "market is closed"), the function must handle the new state gracefully (e.g., return early) instead of reverting. Reverts undo *all* state changes in the transaction, including those made by modifiers.
**Prevention:** When a modifier can transition the contract to a state that would cause the main function to revert, explicitly handle that new state in the function body (e.g., `if (stateChanged) return;`) to ensure the transition persists.
