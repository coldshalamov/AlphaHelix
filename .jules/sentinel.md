...
## 2024-06-07 - Denial of Service via Atomicity in commitBet
**Vulnerability:** A reverting `require` statement was used for an optional reward transfer (`PING_REWARD`) at the end of the `commitBet` function.
**Learning:** Using a reverting `require` on a non-critical auxiliary operation at the end of core logic functions can cause a Denial of Service (DoS) if the external call fails, reverting the entire transaction and blocking primary operations.
**Prevention:** Use a `try/catch` block for non-critical external calls (like optional rewards) to safely handle failures without blocking the main transaction logic.
