...
## 2024-07-02 - Denial of Service via Atomicity in Reward Transfers
**Vulnerability:** Core functions (`commitBet`, `revealBet`, `pingMarket`) were reverting if the non-critical auxiliary operation `token.transfer(msg.sender, PING_REWARD)` failed. This could cause the transaction to fail, preventing markets from closing or users from participating.
**Learning:** External calls like optional token reward transfers should not block primary state changes. If the reward token contract fails the transfer (e.g., due to missing funds or blocklists), the entire transaction reverts.
**Prevention:** Use a `try/catch` block for non-critical auxiliary transfers at the end of core logic functions to handle failures gracefully without blocking primary operations. For standard ERC20, use `try token.transfer(to, amount) returns (bool) {} catch {}`.
