...
## 2024-06-29 - Denial of Service via Atomicity in Auxiliary Transfers
**Vulnerability:** Core logic functions (`commitBet`, `revealBet`, `resolve`, `pingMarket`) used reverting `require(token.transfer(...))` statements for optional reward and fee transfers.
**Learning:** If the market contract lacks sufficient balance or an external token transfer fails, the entire transaction reverts. This "atomicity" causes a DoS, permanently bricking core business logic and potentially trapping user funds (e.g., users cannot reveal their bets and suffer a 100% unrevealed penalty).
**Prevention:** Avoid using reverting `require()` statements for non-critical auxiliary operations at the end of functions. Use `try/catch` blocks (e.g., `try token.transfer(...) returns (bool) {} catch {}`) to handle failures safely without blocking primary operations.
