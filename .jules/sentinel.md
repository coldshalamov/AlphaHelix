...
## 2026-05-25 - Fix CEI Pattern Violations in Event Emissions
**Vulnerability:** Events (`BetCommitted`, `MarketResolved`, `Claimed`, `UnrevealedWithdrawn`) were emitted *after* external interactions (`token.transfer`, `token.transferFrom`).
**Learning:** Although these functions are protected by `nonReentrant` preventing reentrancy attacks, emitting events after external interactions violates the Checks-Effects-Interactions (CEI) pattern. This can lead to out-of-order event logs in complex transaction chains, complicating off-chain indexing and monitoring.
**Prevention:** Always emit events (Effects) *before* making any external calls (Interactions).
