...
## 2024-05-26 - Preconditions Before State Changes
**Vulnerability:** In `commitBet`, `_checkRandomClose` was called before validating the market's phase. If the check triggered a market close, the subsequent validation (`require(s.commitPhaseClosed == 0)`) would revert the transaction, losing the random close outcome and preventing the market from transitioning.
**Learning:** Logic DoS can occur when a function invokes a state-changing side-effect (like closing a market phase) *before* validating preconditions.
**Prevention:** Always validate preconditions (e.g., `require(phaseOpen)`) before executing logic that might alter those same conditions, to prevent unintended transaction reverts.
