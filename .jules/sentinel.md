...
## 2024-06-10 - Fix Denial of Service (DoS) via Atomicity in optional reward transfers
**Vulnerability:** External token transfers for optional auxiliary rewards (like `PING_REWARD` or `fee`) used `require()`, allowing an external transfer failure to revert core state-changing logic in `commitBet`, `revealBet`, and `resolve`.
**Learning:** Using reverting `require()` checks on auxiliary non-critical token transfers violates the "Fail securely" principle by causing a Denial of Service (DoS) that bricks the primary functions if the token transfer fails.
**Prevention:** Always use `try/catch` with explicit `returns (bool)` for optional external rewards within core protocol logic so failures do not block the primary transaction.
