...
## 2024-06-30 - DoS via Atomicity in Auxiliary Transfers
**Vulnerability:** Core market logic functions (`commitBet`, `revealBet`, `resolve`) could be permanently bricked (DoS) if the auxiliary reward or fee token transfers failed (e.g., due to the market contract having insufficient balance or other ERC20 edge cases), as they were wrapped in reverting `require()` statements.
**Learning:** Using `require()` for non-critical token transfers (like optional ping rewards or originator fees) tightly couples the success of core state changes to the success of auxiliary token movements. If the token movement fails, the entire transaction reverts, potentially locking up all funds in the market.
**Prevention:** Always use `try/catch` blocks (or low-level calls) for auxiliary transfers at the end of core logic functions to safely handle failures without blocking primary operations.
