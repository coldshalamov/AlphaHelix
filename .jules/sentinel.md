...
## 2024-03-18 - Prevent Address Grinding in Randomness Source
**Vulnerability:** Pseudo-randomness logic used `msg.sender` as an entropy source, allowing for "address grinding" where attackers generate multiple addresses to find one that produces a favorable hash.
**Learning:** Any user-controlled input, including the transaction sender's address, can be manipulated or pre-calculated in a deterministic way to influence on-chain pseudo-randomness.
**Prevention:** Rely entirely on deterministic, uncontrollable block-level variables (e.g., blockhashes, prevrandao) and protocol state for randomness generation.
