...
## 2026-04-20 - Weak Pseudo-Randomness in Seed Generation
**Vulnerability:** Generating random seeds using only predictable elements like `block.timestamp`, `msg.sender`, and `blockhash` is vulnerable to manipulation.
**Learning:** In Ethereum Proof-of-Stake (post-Merge), generating random seeds using only predictable elements is vulnerable to manipulation. Always include `block.prevrandao` in the `keccak256` payload for a stronger source of pseudo-randomness.
**Prevention:** Always include `block.prevrandao` when generating pseudo-random seeds on-chain to provide a stronger, less predictable entropy source.
