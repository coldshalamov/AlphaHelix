...
## 2024-06-19 - DoS via Atomicity in Reward Transfers
**Vulnerability:** Core logic functions (`commitBet`, `revealBet`, `resolve`) could revert if non-critical auxiliary transfers (ping rewards, originator fees) failed, causing a Denial of Service (DoS).
**Learning:** Using reverting `require()` statements for auxiliary reward transfers makes core business logic vulnerable to atomicity failures. If the reward transfer fails, the entire transaction reverts, bricking the market.
**Prevention:** Use a `try/catch` block for non-critical auxiliary transfers (with explicit `returns (bool)` for ERC20) to safely handle failures without blocking primary operations. Always emit events before interactions to adhere to the CEI pattern.
