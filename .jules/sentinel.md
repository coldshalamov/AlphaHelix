...
## 2026-09-07 - [Commit Revert Loop DoS]
**Vulnerability:** In `commitBet`, calling `_checkRandomClose` before validating `s.commitPhaseClosed == 0` causes the transaction to revert if the random close is triggered. This rolls back the state change, preventing the market from closing and blocking the user's commit, creating a Denial of Service.
**Learning:** State-mutating functions that also act as triggers for time/random phase changes must validate the current phase *before* triggering the phase change, otherwise the phase change itself will cause the validation to fail and revert the entire transaction.
**Prevention:** Always perform current-state validation before calling internal functions that might legitimately advance the state.
