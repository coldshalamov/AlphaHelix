...
## 2025-02-14 - [Address Grinding in Randomness]
**Vulnerability:** `HelixMarket.sol` used `msg.sender` as an entropy source for `checkRandomClose`. This allowed users to grind addresses (creating new addresses with different hashes) to manipulate the market close outcome, forcing it to close (or stay open) at will.
**Learning:** `msg.sender` is user-controllable (at the cost of creating a new account) and should not be used for randomness that affects global state (like market closing). While it prevents replay attacks in user-specific logic (like commit/reveal), it introduces manipulation vectors in global logic.
**Prevention:** Remove `msg.sender` from global randomness seeds. Rely on block properties (`block.prevrandao`, `blockhash`) which are harder for a single user to manipulate without validator collusion.
