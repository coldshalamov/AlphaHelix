...
## 2023-11-20 - [msg.sender in Randomness Pool]
**Vulnerability:** The on-chain global randomness calculation for market closure (`closeHash`) includes `msg.sender`.
**Learning:** Including user-controllable variables like `msg.sender` in randomness pools allows attackers to grind the randomness via proxy contracts (using `CREATE2` to find a `msg.sender` address that results in a desired `closeHash`) or brute-forcing accounts. This violates the principle of not using manipulatable on-chain data for entropy.
**Prevention:** Never include `msg.sender`, `tx.origin`, `tx.gasprice`, or similar user-controllable transaction properties in entropy pools for critical functionality like randomness. Rely on block properties (like `prevrandao`) and market-specific seeds that are not manipulatable at the time of calculation.
