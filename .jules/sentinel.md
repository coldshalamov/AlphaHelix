...
## 2024-05-25 - [Randomness Manipulation via msg.sender]
**Vulnerability:** `HelixMarket.sol` used `msg.sender` as an entropy source for `closeHash`. This allowed attackers to grind sender addresses (creating many accounts) to manipulate the market closing time.
**Learning:** User-controllable fields like `msg.sender` should not be used in dynamic randomness generation tied to immediate outcomes, as they can be manipulated to grind favorable hashes.
**Prevention:** Remove `msg.sender` from dynamic entropy sources. Rely on uncontrollable variables like `block.prevrandao` and historical `blockhash`.
