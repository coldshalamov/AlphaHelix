...
## 2026-03-08 - [Address Grinding in Entropy Sources]
**Vulnerability:** `HelixMarket.sol` used `msg.sender` as an entropy source for `checkRandomClose`. This allowed an attacker to manipulate the random close condition through Address Grinding (e.g., repeatedly deploying temporary contracts until one generated an address that produced a favorable `closeHash` for the current block).
**Learning:** `msg.sender` is an easily manipulable variable, as attackers can programmatically generate many addresses in a single transaction until they find one that passes the randomness check. Combining this with flashbots/private mempools allows risk-free exploitation.
**Prevention:** Completely exclude user-controlled variables (like `msg.sender` or `tx.gasprice`) from entropy sources. Rely entirely on block-level deterministic values (e.g., `block.prevrandao`, `blockhash`) and protocol state to ensure randomness cannot be influenced by the transaction sender.
