...
## 2025-02-27 - Fix Denial of Service via Atomicity in Reward Transfers
**Vulnerability:** Core market functions (commitBet, revealBet, resolve) were using reverting `require()` statements for non-critical auxiliary token transfers (like PING_REWARD and originator fee). If the token transfer failed, the entire transaction reverted, bricking core business logic.
**Learning:** Auxiliary reward or fee transfers should never block primary state-changing operations. A failure in an optional reward should fail silently rather than causing a DoS.
**Prevention:** Use a `try/catch` block (e.g., `try token.transfer(to, amount) returns (bool) {} catch {}`) for non-critical external calls at the end of functions to safely handle failures without reverting the whole transaction.
