...
## 2024-06-18 - Fix DoS via atomic reward transfer in market closure
**Vulnerability:** Core functions (`commitBet`, `revealBet`, `pingMarket`) used a reverting `require` statement for transferring a minor `PING_REWARD`.
**Learning:** If the market contract lacks sufficient balance or the token transfer fails for any reason, the entire transaction reverts. This is a Denial of Service (DoS) vulnerability via atomicity, where failure of a non-critical auxiliary operation (paying a reward) bricks core business logic (committing/revealing bets, closing markets).
**Prevention:** Use a `try/catch` block (`try token.transfer(msg.sender, PING_REWARD) returns (bool) {} catch {}`) for optional or auxiliary external calls at the end of core logic functions to ensure failures do not block the primary operation.
