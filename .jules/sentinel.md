...
## 2025-05-10 - [Critical] Fix DoS vulnerability in commitBet
**Vulnerability:** A Denial of Service (DoS) vulnerability existed in `HelixMarket.sol` where the `commitBet` function could consistently revert. This occurred because `_checkRandomClose` was executed before verifying if the commit phase was open. If the random close hit, it changed the market state, causing the subsequent `require(s.commitPhaseClosed == 0)` check to fail and revert the entire transaction, thereby discarding the market close.
**Learning:** State validation (preconditions) must occur strictly before any function calls that can modify the state being validated, even if those calls are internal side-effects like random closes. Otherwise, valid transactions that trigger a state transition will fail their own validation checks.
**Prevention:** Always validate all initial preconditions (`require` statements based on current state) before executing any internal logic or functions that might alter that state.
