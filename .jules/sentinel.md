...
## 2025-05-22 - [DoS via Reverting Auxiliary Transfer]
**Vulnerability:** Core logic functions (`commitBet`, `revealBet`) and `pingMarket` used a reverting `require(token.transfer(...))` for the non-critical ping reward. If the contract lacked sufficient tokens for the reward or the transfer failed, it reverted the entire transaction, blocking users from committing or revealing bets.
**Learning:** Reverting on failures of non-critical auxiliary operations (like optional rewards) can inadvertently brick core functionalities, leading to Denial of Service (DoS) attacks or states.
**Prevention:** Always use `try/catch` blocks (remembering to specify `returns (bool)` for ERC20 functions) for optional reward transfers to allow core logic to succeed even if the reward fails.
