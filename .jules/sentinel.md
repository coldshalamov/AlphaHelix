...
## 2026-02-26 - GitHub Actions Permissions
**Learning:** GitHub Actions workflows attempting to create comments on Pull Requests must have explicit  defined. The default  permissions may be restrictive depending on repository settings.
**Action:** Always include  for jobs that need to comment on PRs. Note that adding any permission disables the implicit read-all, so  is required for checkout.

## 2026-02-26 - GitHub Actions Permissions
**Learning:** GitHub Actions workflows attempting to create comments on Pull Requests must have explicit `permissions` defined. The default `GITHUB_TOKEN` permissions may be restrictive depending on repository settings.
**Action:** Always include `permissions: { pull-requests: write, contents: read }` for jobs that need to comment on PRs. Note that adding any permission disables the implicit read-all, so `contents: read` is required for checkout.
