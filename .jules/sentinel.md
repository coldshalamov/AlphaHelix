...
## 2024-05-26 - [Randomness Manipulation via Sender]
**Vulnerability:** `HelixMarket.sol` used `msg.sender` as an entropy source for `checkRandomClose`. This allowed attackers to grind addresses (Sybil attack) within the same block to force a market close (or avoid it), manipulating the "random" close time.
**Learning:** Even `msg.sender` introduces user-controllable bias in randomness generation when the check is inexpensive or can be simulated off-chain. If an attacker can choose `msg.sender`, they can choose the outcome if the difficulty is surmountable.
**Prevention:** Exclude all user-controllable values (including `msg.sender`) from on-chain randomness seeds unless specifically designed for commit-reveal. For system-level randomness (like market closing), rely on block properties (`prevrandao`, `timestamp`, `blockhash`) and internal state that cannot be cheaply manipulated in the same transaction.
