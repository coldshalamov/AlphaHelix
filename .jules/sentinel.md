...
## 2025-05-23 - Denial of Service via Atomicity in Reward Transfers
**Vulnerability:** The `commitBet` and `revealBet` functions used a reverting `require()` statement for an optional ping reward transfer at the end of the transaction. If the contract lacked sufficient token balance to pay the reward, the entire transaction would revert, preventing users from committing or revealing their bets (DoS).
**Learning:** Core business logic (like committing or revealing a bet) should never be tightly coupled to the success of non-critical auxiliary operations (like reward payouts). An atomic failure in an auxiliary operation can brick the primary functionality.
**Prevention:** Use a `try/catch` block for optional token transfers at the end of core functions. This ensures that even if the reward transfer fails, the primary state changes (committing/revealing) still succeed.
