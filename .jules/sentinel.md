...
## 2024-05-26 - [Bypassing Random Close via Revert]
**Vulnerability:** A modifier running before `commitBet` could randomly close the market. If it did, the function body's `require(commitPhaseOpen)` would fail, reverting the entire transaction. This undid the market closure, keeping the market open and allowing users to retry their bets until they didn't hit the close condition, bypassing the unpredictable closure mechanism entirely.
**Learning:** Checking termination conditions in a modifier *before* an action that requires the condition to be open creates a paradox where a successful termination reverts its own state changes. If an interaction is allowed to trigger a phase change, the phase change must either not conflict with the interaction's own `require`s, or the interaction must be evaluated before the phase change occurs.
**Prevention:** Move random close / phase termination checks to execute *after* the core logic of the initiating transaction (like accepting the final bet), ensuring state changes from the trigger are committed rather than reverted.
