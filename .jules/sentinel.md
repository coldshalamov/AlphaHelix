...
## 2025-01-15 - DoS via Atomicity in Auxiliary Transfers
**Vulnerability:** The core functions \`commitBet\`, \`revealBet\`, and \`pingMarket\` used a reverting \`require(token.transfer(msg.sender, PING_REWARD))\` at the end of their logic for an optional ping reward transfer.
**Learning:** If the external token transfer failed for any reason, it would revert the entire transaction. For \`commitBet\` and \`revealBet\`, this created a Denial of Service risk that could brick the core business logic (preventing users from revealing and causing them to lose their stakes due to unrevealed penalties).
**Prevention:** Avoid reverting \`require()\` statements for non-critical auxiliary operations. Always use \`try/catch\` blocks (e.g., \`try token.transfer() returns (bool) {} catch {}\`) to handle failures safely without blocking the primary business operations.
