...
## 2026-04-14 - [Weak Randomness in Market Seed Generation]
**Vulnerability:** The initial market seed (`s.closeSeed`) was generated using predictable elements (`block.timestamp`, `msg.sender`, `marketId`, `blockhash`), which could be manipulated by validators.
**Learning:** Post-Merge, `blockhash` and `block.timestamp` are insufficient for pseudo-randomness. Validators can manipulate block times. `block.prevrandao` provides a more robust source of entropy.
**Prevention:** Always include `block.prevrandao` in any `keccak256` payload used for generating randomness or seeds to leverage the stronger entropy of the PoS beacon chain.
