...
## 2026-04-26 - Prevent weak pseudo-randomness manipulation
**Vulnerability:** The `closeSeed` payload generated via `keccak256` relied only on predictable values like `block.timestamp`, `msg.sender`, `marketId`, and `blockhash`.
**Learning:** Using predictable sources for random seeds in Ethereum Proof-of-Stake is vulnerable to manipulation.
**Prevention:** Always include `block.prevrandao` in pseudo-random payloads for stronger security.
