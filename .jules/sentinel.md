...
## 2025-05-15 - Local Secret Overwrite UI Vulnerability
**Vulnerability:** The frontend UI allowed users to re-submit the commit form even if they had already committed on-chain, provided their local secret (`storedBet`) was missing. Submitting the form overwrites the `localStorage` key with a new secret, but the on-chain transaction reverts due to the `Already committed` require statement in the contract. The frontend `catch` block then clears the newly stored secret, completely erasing any chance of recovering the original secret if it had existed.
**Learning:** In "local-first" secret management (storing salts in localStorage), the UI must strictly enforce the contract's state (e.g., querying `hasCommitted` via RPC) to prevent user actions that could corrupt local state. Relying solely on `committedAmount > 0n` or the presence of `storedBet` is insufficient and creates state synchronization vulnerabilities.
**Prevention:** Always directly query authoritative on-chain state (like `hasCommitted`) and use it to disable UI components that handle irreversible secret generation and storage.
