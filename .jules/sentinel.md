...
## 2024-06-22 - [Denial of Service via Auxiliary Token Transfers]
**Vulnerability:** Core functions `commitBet`, `revealBet` and `pingMarket` used a reverting `require` statement for an optional `PING_REWARD` token transfer at the end of their logic. If the transfer failed, the entire transaction reverted, bricking core market interactions.
**Learning:** Auxiliary or non-critical operations (like optional rewards) should never be allowed to revert the entire transaction of core business logic (Denial of Service via Atomicity).
**Prevention:** Use `try/catch` blocks (`try token.transfer(...) returns (bool) {} catch {}`) for non-critical external calls to safely handle failures without blocking primary operations.
