...
## 2026-05-26 - Log-based Reentrancy via Out-of-Order Events
**Vulnerability:** External contract interactions (like token transfers) were executing before the corresponding state events (like `MarketResolved` or `BetCommitted`) were emitted. Even with `nonReentrant` guards, this Checks-Effects-Interactions (CEI) violation allows the external token contract's `transferFrom` or `transfer` hooks to trigger state changes in other systems *before* the market records the event, enabling out-of-order logging attacks.
**Learning:** Event emissions are a form of state change (Effects). They must always be committed *before* external calls (Interactions). A `nonReentrant` modifier only protects against synchronous reentrancy into the same contract, not against asynchronous log manipulation or off-chain indexer confusion caused by incorrect event ordering.
**Prevention:** Strictly enforce the Checks-Effects-Interactions pattern for *all* state changes, including `emit` statements. Move all `emit` commands immediately prior to any external calls (e.g., `token.transfer`).
