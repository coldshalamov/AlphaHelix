...
## 2024-05-18 - Fix Self-Reverting Transaction DoS on Random Close
**Vulnerability:** Calling `_checkRandomClose` at the beginning of `commitBet` modifies state to close the commit phase if random close condition is met. However, immediately after, `commitBet` asserts that the commit phase is still open (`require(s.commitPhaseClosed == 0)`). This causes a transaction revert when the market successfully randomly closes, acting as a temporary Denial of Service (DoS) preventing users from committing.
**Learning:** State validation must happen before state changes that could inadvertently trigger those validations to fail in the same transaction. Moving state-changing side-effects like `_checkRandomClose` to the end of functions prevents self-inflicted reverts.
**Prevention:** Always place side-effect triggers (like random closes) after the primary function validation logic, or evaluate validation checks before state is potentially modified by internal helper functions.
