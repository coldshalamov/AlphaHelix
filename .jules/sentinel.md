...
## 2026-05-15 - State Revert on Side-Effect Logic
**Vulnerability:** In `commitBet`, a precondition check reverted the transaction *after* a successful state-changing side effect (random close) was triggered earlier in the function, causing a logic DoS that prevented the market from ever closing during a bet.
**Learning:** Validating preconditions after executing a state-changing side-effect can cause unintended reverts that roll back the side-effect itself.
**Prevention:** Always validate all preconditions before executing logic that alters those same conditions or triggers side-effects.
