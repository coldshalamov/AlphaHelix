...
## 2026-04-16 - [Predictable Pseudo-Randomness Seed]
**Vulnerability:** The `closeSeed` generation for random-close markets relied only on legacy predictable elements (`block.timestamp`, `msg.sender`, and `blockhash`).
**Learning:** In Proof-of-Stake Ethereum, relying on these elements without `block.prevrandao` makes the initial seed generation more vulnerable to validator manipulation.
**Prevention:** Always include `block.prevrandao` in `keccak256` payloads when generating on-chain pseudo-randomness.
