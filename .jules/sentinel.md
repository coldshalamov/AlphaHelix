...
## 2024-06-26 - Fix DoS via atomicity in reward transfers
**Vulnerability:** A reverting `require()` statement was used for non-critical auxiliary reward transfers (`PING_REWARD`) at the end of core logic functions (`commitBet`, `revealBet`, `pingMarket`).
**Learning:** If the token transfer failed (e.g., due to low contract balance), the entire transaction reverted, causing a DoS and effectively bricking the market's core operations.
**Prevention:** Use a `try/catch` block for external/optional transfers that shouldn't block primary core business logic.
