...
**Vulnerability:** `HelixMarket.sol` includes `msg.sender` in its on-chain randomness calculations (`_checkRandomClose`).
**Learning:** Including user-controllable variables like `msg.sender` in entropy pools allows attackers to grind the randomness via proxy contracts or `CREATE2` to force desired outcomes (such as forcing a market close at an optimal time).
**Prevention:** Never include user-controllable variables like `msg.sender` or `tx.origin` in entropy pools meant for global, unpredictable on-chain randomness.
