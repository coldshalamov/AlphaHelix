...
## 2024-05-26 - [Randomness Grinding via msg.sender]
**Vulnerability:** The PRNG for market closure included `msg.sender` as an entropy source. This allowed attackers to grind the result by generating and simulating transactions from numerous addresses until a favorable hash was produced.
**Learning:** Including transaction-specific, user-controllable variables like `msg.sender` in an on-chain PRNG entropy pool breaks the unpredictability of the result. It allows cost-free off-chain grinding of the hash.
**Prevention:** Remove user-controllable transaction variables like `msg.sender` from dynamic entropy sources. Rely on block-level values like `block.prevrandao` and `blockhash` that the user cannot freely manipulate.
