...
## 2024-11-20 - Prevent Randomness Grinding
**Vulnerability:** The global on-chain randomness calculation in `HelixMarket` included the user-controllable `msg.sender` variable.
**Learning:** Including user-controllable variables in entropy pools allows attackers to grind the randomness via proxy contracts or `CREATE2` to force desired outcomes.
**Prevention:** Never include `msg.sender` or `tx.origin` in global randomness generation.
