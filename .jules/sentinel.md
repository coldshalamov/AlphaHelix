...
## 2024-06-04 - DoS via Atomicity in Auxiliary Rewards
**Vulnerability:** Core business logic (committing, revealing bets) would revert if the non-critical auxiliary operation of transferring a 'Ping Reward' failed.
**Learning:** Using reverting `require` statements for auxiliary side-effects like rewards or notifications can lead to a 'Denial of Service via Atomicity', bricking the core logic if the external transfer fails.
**Prevention:** Always use `try/catch` blocks (or low-level calls) when interacting with external contracts for non-critical auxiliary operations at the end of core functions.
