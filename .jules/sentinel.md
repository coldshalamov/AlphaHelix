...
## 2026-04-23 - Predictable Random Seed Generation Post-Merge
**Vulnerability:** The `s.closeSeed` was generated using only predictable elements (`block.timestamp`, `msg.sender`, `marketId`, `blockhash`), omitting `block.prevrandao`.
**Learning:** In post-Merge Ethereum, `block.timestamp` and `blockhash` are insufficient for strong pseudo-randomness; `block.prevrandao` must be included as a beacon chain randomness source to mitigate manipulation risks.
**Prevention:** Always include `block.prevrandao` in `keccak256` payloads when generating on-chain pseudo-random seeds.
