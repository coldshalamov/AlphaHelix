...
## 2026-05-12 - Prevent State Rollback DoS on Auto-Close Triggers
**Vulnerability:** A "Logic DoS" existed where state-changing trigger functions (like `_checkRandomClose`) were called *before* precondition validations (like `require(commitPhaseClosed == 0)`).
**Learning:** If a transaction meets the conditions to trigger a market phase transition, updating the state first causes subsequent strict lifecycle validations to fail. The resulting `require` revert rolls back the entire transaction, meaning the triggered phase transition is discarded and the market can never successfully close via normal interaction.
**Prevention:** Always validate transaction lifecycle preconditions *before* executing side-effect logic that might alter those same lifecycle conditions, or cache the precondition state before evaluating the trigger.
