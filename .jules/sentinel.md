...
## 2025-05-23 - [Denial of Service via Atomicity in Auxiliary Rewards]
**Vulnerability:** Core functions `commitBet` and `revealBet` used a reverting `require(token.transfer(...))` statement to pay out an optional `PING_REWARD` at the end of their execution. If the transfer failed, the entire transaction reverted, bricking core business logic.
**Learning:** Auxiliary or non-critical operations (like optional reward transfers) should never block core functionality. Even if the state is correctly updated before the external call, a reverting `require` on that call will undo the entire transaction, causing a Denial of Service.
**Prevention:** Use a `try/catch` block or low-level calls to safely handle failures for non-critical auxiliary operations at the end of main functions.
