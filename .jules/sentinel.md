...
## 2026-02-22 - [DoS of Market Closure via Revert]
**Vulnerability:** The `checkRandomClose` modifier in `HelixMarket.sol` closes the market (state transition) if random conditions are met. However, the decorated `commitBet` function immediately checked `require(commitPhaseClosed == 0)`, causing the transaction to revert if the market *just* closed. This revert rolled back the closure, leaving the market open and allowing the user to lose gas without effect, effectively creating a DoS of the closing mechanism.
**Learning:** When a modifier performs a state transition (like closing a market) that invalidates a precondition of the main function, the function must handle the new state gracefully (e.g., return early) rather than reverting. Reverting undoes the modifier's work, which is critical if the modifier's purpose is to transition the system state.
**Prevention:** In functions wrapped by state-changing modifiers, explicitly check if the state changed during the transaction (e.g., `state.closedTimestamp == block.timestamp`) and return early instead of reverting, ensuring the state transition persists.
