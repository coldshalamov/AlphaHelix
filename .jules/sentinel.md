...
## 2024-05-26 - [Randomness Manipulation via Proxy Grinding]
**Vulnerability:** `HelixMarket.sol` used `msg.sender` as an entropy source for `_checkRandomClose`. This allowed an attacker to write a proxy factory that deploys proxy contracts via `CREATE2`, checks if the proxy's address (`msg.sender` from the proxy's perspective) results in a favorable `closeHash`, and if not, reverts the transaction. This allowed attackers to grind the randomness pool to force a market close at a desired time.
**Learning:** Any user-controllable input, including the transaction's `msg.sender` (which can be deterministically generated via `CREATE2` and verified on-chain before committing), completely breaks the security of on-chain pseudo-randomness.
**Prevention:** Never include user-controllable variables like `msg.sender` or `tx.origin` in the entropy pool when calculating on-chain global randomness. Rely exclusively on network-level randomness like `block.prevrandao` and historical `blockhash`.
