...
## 2024-05-26 - [Predictable Randomness Seed]
**Vulnerability:** Generating random seeds using only predictable elements like `block.timestamp`, `msg.sender`, and `blockhash` is vulnerable to manipulation by block builders.
**Learning:** In `HelixMarket.sol`'s `_submitStatementInternal`, `s.closeSeed` was generated without incorporating `block.prevrandao`, making the market close entropy easier to predict or manipulate.
**Prevention:** In Ethereum Proof-of-Stake (post-Merge), always include `block.prevrandao` in the `keccak256` payload when generating on-chain random seeds to leverage the stronger pseudo-randomness provided by the beacon chain.
