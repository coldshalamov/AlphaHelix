...
## 2024-07-29 - Decorative Spinner Redundancy
**Learning:** The `Spinner` component was using `role="status"` and `aria-label="Loading"`, but it is exclusively used next to descriptive text (like 'Committing...') inside buttons across the app. This caused screen readers to redundantly announce 'Loading Committing...'.
**Action:** Always apply `aria-hidden="true"` to decorative loading icons when the adjacent text already explicitly describes the loading state to screen readers.
