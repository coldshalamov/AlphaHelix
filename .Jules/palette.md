...
## 2026-04-28 - [Native Form Accessibility]
**Learning:** Using `div` wrappers with `onClick` handlers for financial/transaction inputs breaks native keyboard accessibility, preventing users from pressing 'Enter' to submit.
**Action:** Wrap transaction inputs in a semantic `<form onSubmit={...}>` with `<button type="submit">` to restore default native keyboard interactions.
