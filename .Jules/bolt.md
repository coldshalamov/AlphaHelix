...
## 2024-05-30 - [Manual Hex Conversion Anti-Pattern]
**Learning:** Manual byte-array to hex string conversion using `Array.from(buffer).map(...).join('')` is an anti-pattern causing excessive intermediate allocations and GC pressure.
**Action:** Use native utilities like `bytesToHex` from `viem` for optimal memory/CPU performance on the frontend.
