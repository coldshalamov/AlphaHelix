...
## 2024-08-25 - Use native viem utility for hex conversion
**Learning:** Manual mapping of Uint8Array to hex strings (e.g., Array.from(buffer).map(...).join('')) causes intermediate array allocations, string creations, and garbage collection overhead.
**Action:** Always use native utilities like viem's bytesToHex() to eliminate this overhead when converting byte arrays to hex strings in the frontend.
