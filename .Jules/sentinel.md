...
## 2025-05-23 - [Uninitialized Timeline Variables Bypass]
**Vulnerability:** In commit-reveal markets with dynamically assigned phase boundaries (like `s.revealEndTime`), initializing the boundary variable to `0` and relying only on a `block.timestamp > boundary` check allows an attacker to bypass the phase check entirely. Because `block.timestamp` is always greater than `0`, an attacker could invoke post-phase actions (like withdrawing unrevealed stakes) prematurely during the commit phase, effectively bypassing the 100% burn penalty.
**Learning:** Initializing critical timeline variables to `0` creates a dangerous state where timestamp comparisons (e.g., `block.timestamp > s.revealEndTime`) succeed unexpectedly before the timeline is actually set.
**Prevention:** Always verify that dynamically set phase boundaries have been initialized (e.g., `s.revealEndTime != 0`) before using them in timestamp comparisons to enforce phase transitions.
