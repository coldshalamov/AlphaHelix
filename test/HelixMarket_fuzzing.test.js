...
describe("HelixMarket - Fuzzing & Invariant Tests", function () {
  ...
  describe("Pool Accounting Invariants", function () {
    it("Total pool = yesPool + noPool + unalignedPool (invariant)", async function () {
      const { market, token, userA, userB, userC, userD } = await loadFixture(
        deployHelixMarketFixture,
      );

      await market
        .connect(userA)
        .submitStatement("ipfs://invariant", biddingDuration, revealDuration);
      const marketId = 0;
      ...
      await market
        .connect(userB)
        .commitBet(marketId, buildCommit(1, 111, userB), amounts[0]);
      await market
        .connect(userC)
        .commitBet(marketId, buildCommit(0, 222, userC), amounts[1]);
      await market
        .connect(userD)
        .commitBet(marketId, buildCommit(2, 333, userD), amounts[2]);
      await market
        .connect(userA)
        .commitBet(marketId, buildCommit(1, 444, userA), amounts[3]);
      ...
      const totalPool =
        statement.yesPool + statement.noPool + statement.unalignedPool;
    ...
    it("Payouts + fees = total pool (conservation of funds)", async function () {
      ...
      await market
        .connect(userD)
        .commitBet(marketId, buildCommit(2, 333, userD), unalignedAmount);
    ...
    it("Multiple winners share pool correctly (pro-rata)", async function () {
      const { market, token, userA, userB, userC, userD, userE } =
      ...
      await market
        .connect(userB)
        .commitBet(marketId, buildCommit(1, 111, userB), yesAmounts[0]);
      await market
        .connect(userC)
        .commitBet(marketId, buildCommit(1, 222, userC), yesAmounts[1]);
      await market
        .connect(userD)
        .commitBet(marketId, buildCommit(1, 333, userD), yesAmounts[2]);
      await market
        .connect(userE)
        .commitBet(marketId, buildCommit(0, 444, userE), noAmount);
      ...
      const totalPool =
        yesAmounts[0] + yesAmounts[1] + yesAmounts[2] + noAmount;
  ...
  describe("Extreme Pool Ratios", function () {
    it("Handles 1:1000000 pool ratio (tiny YES vs huge NO)", async function () {
      ...
      await market
        .connect(userB)
        .commitBet(marketId, buildCommit(1, 111, userB), tinyYes);
      await market
        .connect(userC)
        .commitBet(marketId, buildCommit(0, 222, userC), hugeNo);
    ...
    it("Handles exact tie with large numbers", async function () {
      ...
      await market
        .connect(userA)
        .submitStatement("ipfs://large-tie", biddingDuration, revealDuration);
      const marketId = 0;
      ...
      await market
        .connect(userB)
        .commitBet(marketId, buildCommit(1, 111, userB), largeAmount);
      await market
        .connect(userC)
        .commitBet(marketId, buildCommit(0, 222, userC), largeAmount);
    ...
    it("Handles minimum stake (1 wei)", async function () {
      ...
      await market
        .connect(userA)
        .submitStatement("ipfs://min-stake", biddingDuration, revealDuration);
      const marketId = 0;
      ...
      await market
        .connect(userB)
        .commitBet(marketId, buildCommit(1, 111, userB), minStake);
      await market
        .connect(userC)
        .commitBet(marketId, buildCommit(0, 222, userC), normalStake);
  ...
  describe("Maximum Participants", function () {
    it("Handles 6 participants with mixed choices", async function () {
      const { market, token, owner, userA, userB, userC, userD, userE } =
        await loadFixture(deployHelixMarketFixture);

      await market
        .connect(userA)
        .submitStatement(
          "ipfs://max-participants",
          biddingDuration,
          revealDuration,
        );
      const marketId = 0;
      ...
      for (let i = 0; i < users.length; i++) {
        await market
          ...
          .commitBet(
            marketId,
            buildCommit(choices[i], 1000 + i, users[i]),
            amounts[i],
          );
      }
      ...
      for (let i = 0; i < users.length; i++) {
        await market
          .connect(users[i])
          .revealBet(marketId, choices[i], 1000 + i);
      }
  ...
  describe("Griefing Attack Resistance", function () {
    it("Cannot grief by committing but never revealing", async function () {
      ...
      await market
        .connect(userA)
        .submitStatement("ipfs://grief-test", biddingDuration, revealDuration);
      const marketId = 0;
      ...
      await market
        .connect(userB)
        .commitBet(marketId, buildCommit(1, 111, userB), honestAmount);
      await market
        .connect(userC)
        .commitBet(marketId, buildCommit(0, 222, userC), grieferAmount);
  ...
  describe("Edge Cases", function () {
    ...
    it("Prevents double claiming", async function () {
      ...
      await expect(market.connect(userB).claim(marketId)).to.be.revertedWith(
        "No winning bet",
      );
    });
  });
