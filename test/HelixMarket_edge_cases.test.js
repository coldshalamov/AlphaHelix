...
describe("HelixMarket Edge Cases", function () {
  ...
  it("reverts malformed reveal (wrong salt)", async function () {
    const { market, originator, userA } = await loadFixture(
      deployHelixMarketFixture,
    );

    await market
      .connect(originator)
      .submitStatement("ipfs://bad-salt", biddingDuration, revealDuration);
    const marketId = 0;

    const amount = ethers.parseEther("10");
    await market
      .connect(userA)
      .commitBet(marketId, buildCommit(1, 111, userA), amount);

    await time.increase(biddingDuration + 1);

    await expect(
      market.connect(userA).revealBet(marketId, 1, 112),
    ).to.be.revertedWith("Invalid hash/reveal");
    await expect(market.connect(userA).revealBet(marketId, 1, 111)).to.not.be
      .reverted;
  });
  ...
  it("reverts malformed reveal (invalid choice even if hash matches)", async function () {
    ...
    await market
      .connect(originator)
      .submitStatement("ipfs://bad-choice", biddingDuration, revealDuration);
    const marketId = 0;
    ...
    await market
      .connect(userA)
      .commitBet(marketId, buildCommit(invalidChoice, salt, userA), amount);

    await time.increase(biddingDuration + 1);

    await expect(
      market.connect(userA).revealBet(marketId, invalidChoice, salt),
  ...
  it("reverts double-claim attempts", async function () {
    ...
    await market
      .connect(originator)
      .submitStatement("ipfs://double-claim", biddingDuration, revealDuration);
    const marketId = 0;
    ...
    await market
      .connect(userA)
      .commitBet(marketId, buildCommit(1, 777, userA), yesAmount);
    await market
      .connect(userB)
      .commitBet(marketId, buildCommit(0, 888, userB), noAmount);
  ...
  it("handles zero NO pool (YES wins with no opposition)", async function () {
    ...
    await market
      .connect(originator)
      .submitStatement("ipfs://zero-no", biddingDuration, revealDuration);
    const marketId = 0;

    const yesAmount = ethers.parseEther("10");
    await market
      .connect(userA)
      .commitBet(marketId, buildCommit(1, 1111, userA), yesAmount);
