...
describe("HelixMarket Security: Address Grinding", function () {
  ...
  it("FIXED: Hash is independent of msg.sender (Address Grinding Prevented)", async function () {
    // Advance time to allow random close check
    await ethers.provider.send("evm_increaseTime", [3600]);
    await ethers.provider.send("evm_mine");

    // Check previewCloseCheck with User 1
    const res1 = await market.connect(user1).previewCloseCheck.staticCall(marketId);

    // Check previewCloseCheck with User 2
    ...
    // Fix Check: Hashes should be identical regardless of sender
