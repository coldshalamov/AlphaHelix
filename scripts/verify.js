...
function getRequiredAddress(name, envKey, addresses, addressesKey) {
  ...
  if (!value) {
    throw new Error(
      `Missing ${name} address. Provide ${envKey} or set ${addressesKey} in frontend/src/config/contracts.json.`,
    );
  }
  return value;
}
...
async function main() {
  ...
  const tokenAddress = getRequiredAddress(
    "AlphaHelixToken",
    "TOKEN_ADDR",
    addresses,
    "AlphaHelixToken",
  );
  const reserveAddress = getRequiredAddress(
    "HelixReserve",
    "RESERVE_ADDR",
    addresses,
    "HelixReserve",
  );
  const marketAddress = getRequiredAddress(
    "HelixMarket",
    "MARKET_ADDR",
    addresses,
    "HelixMarket",
